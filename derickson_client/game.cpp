...
DLLEXPORT int createGame()
{
  ...
  login(socket, last_username, last_password);
...
void parseUnit(_Unit* object, sexp_t* expression)
{
  ...
  sub = sub->next;
  object->ownerIndex = atoi(sub->val);
  sub = sub->next;
